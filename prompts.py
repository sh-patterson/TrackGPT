...
# 2.  **Speaker Identification (Per Bullet):** Identify the primary person speaking the text selected for the `Body:` section below. Use their name (e.g., "KANYE WEST") or role (e.g., "INTERVIEWER", "CEO") in ALL CAPS. If the speaker is the video's main subject ({target_name}), use their name (ALL CAPS). If unknown, use "UNKNOWN SPEAKER".
...
#     *   This passage **should primarily feature words spoken by the identified `Speaker:`** but **MAY include necessary surrounding context**, such as brief introductory phrases from a reporter or interviewer (e.g., "When asked about X, {target_name} stated:") if it directly precedes the key statement and aids understanding.
 ...
 *   **Summarize Core Fact (PAST TENSE):** Concisely summarize the absolute core finding, action, or statement (1-2 sentences maximum), **accurately attributing it to the correct actor** ({target_name}, interviewer, source, etc.). Use **simple PAST TENSE** verbs (e.g., Said, Stated, Announced, Claimed, Revealed, Launched, Criticized, Denied, Confirmed, Met, Asked).
 ...
 *   **Optional Short Quote Snippet:** You MAY include a *very short* (under 10 words), impactful, verbatim quote snippet in double quotes within the summary *if* it *is* the core point AND is directly spoken by the actor named in the headline (e.g., `Kanye West Described New Project As "Revolutionary".`). Use sparingly.
...
@@DELIM@@
**Speaker:** [Primary Speaker of Body Text (ALL CAPS)]
@@DELIM@@
